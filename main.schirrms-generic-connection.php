            ...
            foreach ($aVirtInterfaces as $aVirt) {
                ...
                foreach ($aVirt as $aGen) {
                    if (array_key_exists($aGen['GenInt'], $aConnDevDepends)) {
                        ...
                        if (array_key_exists($aGen['GenInt'], $aDirectConnDevDepends)) {
                            unset($aDirectConnDevDepends[$aGen['GenInt']]);
                        }
                    }
                }
                $bPush = TRUE;
                foreach ($aDependDevice as $aValid) {
                    if ($aValid['Redundancy'] == $aTmp['Redundancy'] && $aValid['remoteDev'] == $aTmp['remoteDev']) {
                        $bPush = FALSE;
                    }
                }
                if ($bPush && $aTmp['Redundancy'] != '') {
                    $aDependDevice[] = $aTmp;
                }
            }
            // there still a cleanup to do in case of more than one connection between two devices
            $aDirectConnectDevices = array();
            foreach ($aDirectConnDevDepends as $nLocalInt) {
                $aDirectConnectDevices[$nLocalInt['remoteDev']] = '';
            }
